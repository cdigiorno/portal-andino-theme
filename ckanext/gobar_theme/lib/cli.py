...
class CleanDatastoreCommand(cli.CkanCommand):
    summary = ""

    def command(self):
        self._load_config()

        LOGGER.info("Realizando limpieza del Datastore")
        ...
        # Se borrarán los recursos del Datastore que no figuren en `node_resource_ids`
        ...
        while datastore_resources.get('total') > current_offset:
            for datastore_resource in datastore_resources.get('records'):
                ...
                if datastore_resource_id != "_table_metadata" and datastore_resource_id not in node_resource_ids:
                    try:
...
class UpdateDatastoreCommand(cli.CkanCommand):
    ...
    def command(self):
        ...
        LOGGER.info("Actualizando el Datastore")
